...
v <- rbind(c(runif(max=3, n1),sample(c(0.5, 1, 1.5), N, T)),c(runif(max=3, n1),sample(c(0.2, 0.3, 0.4, 0.6, 1.5), N, T)))
w <- v
...
plot(timestamps[1:27000], W_history[1:27000,1]/(W_history[1:27000,1]+W_history[1:27000,2]), type='l', xlab='t',ylab='W_1(t)/W_Sigma(t)', col = 'red',
     cex.lab=1.5, cex.axis=2, ylim = c(0, 1))
abline(h=1/4, lty=4, lwd=3)
plot(timestamps[1:27000], W_history[1:27000,2], type='l', xlab='t',ylab='', col='red',
     cex.lab=1.5, cex.axis=2)
lines(timestamps[1:27000], 3/4*(W_history[1:27000,1]+W_history[1:27000,2]), type='l', col='blue')
legend(110, legend=c("W_1(t)", "p_1*gamma_1*W_Sigma(t)"), col=c("red", "blue"),lty=1,
       cex=1.75, lwd = 4)
